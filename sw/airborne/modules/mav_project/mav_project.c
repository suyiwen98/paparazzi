...
#define PRINT(string, ...) fprintf(stderr, "[mav_project->%s()] " string,__FUNCTION__ , ##__VA_ARGS__)
...
static void optical_flow_cb(uint8_t __attribute__((unused)) sender_id,
                            int16_t __attribute__((unused)) error,
                            int16_t __attribute__((unused)) foe_x,
...
void mav_project_init(void) {
    ...
    AbiBindMsgOPTICAL_FLOW2(ABI_BROADCAST, &optical_flow_ev, optical_flow_cb);
}

void mav_project_periodic(void) {
